...
const ItemCard1 = ({ index, address, identifier, order }: { index: number, address: string, identifier: string, order?: OrderWrapperInterface }) => {
    ...
    const nft = useNFTMetadata(address, identifier)
    ...
    return (
        <article className={"relative w-full shadow-lg bg-zinc-50 border border-gray-200 rounded-lg cursor-pointer hover:-translate-y-1 hover:shadow-2xl"} >
            <div className="flex justify-center overflow-x-hidden w-full aspect-square rounded-t-lg relative bg-slate-200">
                <a href={'/#/collection/' + address + '/' + identifier.toString()}>{(nft.imageURL == '' && nft.animationURL != '') || playVideo ? <video className="bg-black h-full aspect-video object-contain" src={nft.animationURL} autoPlay muted loop /> : <img className="h-full object-contain" src={nft.imageURL} />}</a>
                {nft.animationURL != '' && nft.imageURL != '' ? <div onClick={tooglePlayVideo} className='absolute bottom-1 right-1 bg-white hover:shadow-2xl rounded-full w-8 h-8 p-2 fill-gray-400'>{playVideo ? <Pause className='' /> : <Play />}</div> : <></>}
            </div>
            <a href={'/#/collection/' + address + '/' + identifier.toString()} className="mt-2 pl-2 mb-2 flex justify-between h-16">
                <div className=' flex flex-col w-full'>
                    <div className="flex flex-row items-center">
                        <div className="flex flex-col w-full">
                            <p className="text-2xs font-semibold text-gray-600 mb-0 text-clip h-min ">{nft.collectionName}</p>
                            <p className="text-xs font-semibold text-gray-900 mb-0 text-clip ">{nft.name}</p>
                        </div>
